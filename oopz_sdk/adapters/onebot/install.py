...
def install_onebot_v11(bot: OopzBot) -> Any | None:
    ...
    adapter = OneBotV11Adapter(
        ...
        self_oopz_id=getattr(config, "self_id", "") or bot.config.person_uid,
    ...
    if getattr(config, "auto_start_server", True):
        server_config = OneBotV11ServerConfig(
            ...
            enable_http_post=getattr(config, "enable_http_post", True),
            enable_ws_reverse=getattr(config, "enable_ws_reverse", True),
            http_post_urls=list(getattr(config, "http_post_urls", []) or []),
            http_post_timeout=getattr(config, "http_post_timeout", 0.0),
